...
def parse_args():
    ...
    parser.add_argument(  # default: "s*"
        "--pattern",
        type=str,
        help="Pattern for the image generation",
        default="s*",
    )
    parser.add_argument(  # override alpha/beta if desired
        "--alpha",
        type=float,
        default=None,
    ...
    parser.add_argument(  # different per-block alpha settings -> deprecated, only using non-linear scheduling now
        "--alpha_blocks",
        type=str,
        default=None,
        help='JSON for per-block alpha, e.g. {"down":4,"mid":4,"up":4}',
    )
    parser.add_argument(  # different per-block beta settings -> deprecated, only using non-linear scheduling now
        "--beta_blocks",
        type=str,
        default=None,
        help='JSON for per-block beta, e.g. {"down":3,"mid":3,"up":3}',
    )
    parser.add_argument(  # different scheduling options: linear, cosine, exp
        "--schedule_type",
        type=str,
        choices=["linear", "cosine", "exp"],
        default="linear",
        help="Time scheduling curve for K-LoRA scaling",
    )
    parser.add_argument(  # the steepness of the exp curve (if using exp scheduling)
        "--schedule_strength",
        type=float,
        default=3.0,
        help="Exp curve steepness (ignored if not exp)",
    )
    parser.add_argument(
        "--num_images",
        type=int,
...
if pattern == "s*":
    alpha = 1.5  # paper default values: 1.5
    beta = alpha * 0.85
else:
    alpha = 1.5
    beta = 0.5
