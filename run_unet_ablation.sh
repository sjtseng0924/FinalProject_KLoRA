...
ALPHA_GRID_DOWN=(2 2 3 4)
ALPHA_GRID_MID=(2 2 3 4)
ALPHA_GRID_UP=(2 3 4 5)
BETA_GRID_DOWN=(1.5 1.5 2 3)
BETA_GRID_MID=(1.5 1.5 2 3)
BETA_GRID_UP=(1.5 2.5 3 4)
...
# ---- Baseline runs (global alpha/beta, no per-block) ----
for a_base in "${BASELINE_ALPHAS[@]}"; do
  b_base=$(python - <<PY
a=$a_base
print(f"{a*0.85}")
PY
)
  out_dir="$BASE_OUT/baseline_a${a_base}_b${b_base}"
  echo "==> Baseline $out_dir"
  python inference_sd.py \
    ...
    --alpha "$a_base" \
    --beta "$b_base" || { echo "[WARN] baseline inference failed at $out_dir"; continue; }
